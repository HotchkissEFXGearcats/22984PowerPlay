...
public class GOOD_ThreadedMecanum extends LinearOpMode {
    ...
    //lift variables
    private int min = 20;
    private int max = 2730;
    private int liftZero;
    private int bottomBuffer = min + 300;
    private int topBuffer = max + 300;
    private double slow = 0.1;
    private double fast = 0.8;

    boolean headless = false;
    double headlessWait = 0;
    
    boolean coreUp = false;
    boolean coreDown = false;
    
    double corePowerUp = 0.9;
    double corePowerDown = 0.5;
    ...
    public void runOpMode() {
        ...
        // Create separate threads for different functionalities
        Thread liftThread = new Thread(() -> {
            while (opModeIsActive()) {
                moveLift(-0.4 * gamepad2.right_stick_y);
                autoLift(1.0);
            }
        });
        ...
        Thread coreMotorThread = new Thread(() -> {
            while (opModeIsActive()) {
                coreMotor.setPower(gamepad1.right_trigger - gamepad1.left_trigger);
                
                if (coreUp) {
                    if ((coreMotor.getCurrentPosition() < -50) /*&& leftLift.getCurrentPosition() > bottomBuffer*/) { //1000
                        // flip motor up
                        while (coreMotor.getCurrentPosition() < -15) {
                            coreMotor.setPower(corePowerUp);
                        }
                ...
                if (coreDown) {
                    if ((coreMotor.getCurrentPosition() > -25) && leftLift.getCurrentPosition() < topBuffer ){ //1300
                        // flip motor down
                        while (coreMotor.getCurrentPosition() > -20) {
                            coreMotor.setPower(-corePowerDown);
                        }
                        coreMotor.setPower(0.0);
                    }
                    coreDown = false;
